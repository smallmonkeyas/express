...
async function renderFreshData(dataTooltipPArr) {
  ...
  originVQTTime =
    res.result.tags.quality === "0" && res.result.tags.originalTime > 0
      ...
      : 0;
  if (
    res.result.tags.quality === "0" &&
    res.result.tags.originalTime > 0 &&
    res.result.tags.status === "0" &&
    res.result.tags.serverTime > 0
  ) {
    originVQTTime = Math.max(res.result.tags.serverTime,res.result.tags.originalTime)
  }
...
async function getLastFreshTime(objName, propName, originVQTTime) {
  ...
  lastFreshTime = Math.max(lastFreshTime,originVQTTime)  

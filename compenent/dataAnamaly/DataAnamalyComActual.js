 ...
 * @LastEditTime: 2021-09-09 09:59:10
...
async function fetchAlarmListByInterface(epcode, starttime, endtime) {
    ...
    const response = await fetch(
        `http://${window.location.hostname}:8999/serverapi/event/total/alarmhis?epcode=${epcode}&alarmtype=1,4,8,11,12&begintime=${starttime}&endtime=${endtime}`,
        requestOptions
    );
...
class AnamalyDetectChartTab extends Component {
    ...
    fetchAlarmList = () => {
        ...
        fetch(
            `http://${window.location.hostname}:8999/serverapi/event/total/alarmhis?epcode=${this.epcode}&alarmtype=1,2,4,5,8,11,12&begintime=${this.starttime}&endtime=${this.endtime}`,
            requestOptions
        )
            .then((response) => response.text())
            .then((result) => {
    ...
    selectFilterChange = (value) => {
        ...
        setTimeout(() => {
            ...
            this.arrFilterSelectParam = {
                ...
                equipmentFilter: this.equipmentFilterRef.current.rcSelect.state.value[0],

...
const paramTableHeaderWidth = [80, 80, 60]
const columnsParam = []
...
async function fetchAlarmListByInterface(epcode, starttime, endtime) {
    ...
    const response = await fetch(
        ...
        `http://${window.location.hostname}:8999/serverapi/event/total/alarmhis?epcode=${epcode}&alarmtype=1,4,11,12&begintime=${starttime}&endtime=${endtime}`,
...
async function getHisDataByhttpInterface(selectedRows, epcode) {
    const hisEndTime = moment(selectedRows.starttime).add(10, "minutes").format("YYYY-MM-DD HH:mm:ss")
...
class AnamalyDetectChartTab extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            timeRangeValue: [
                moment(moment().subtract(12, "hours"), "YYYY-MM-DD HH:mm:ss"),
        ...
        this.starttime = moment().subtract(12, "hours").format("YYYY-MM-DD HH:mm:ss")
        this.initColumn()
    }
    ...
    fetchAlarmListAsync = () => {
        ...
        getAlarmInfoPromise.then((data) => {
            ...
            // if (data.length) {
            //     this.rowCheckSelect(this.state.rowkey, data[0])
            // }
            this.setState({
                dataSource: data,
                loading: false
    ...
    fetchAlarmList = () => {
        ...
        fetch(
            ...
            `http://${window.location.hostname}:8999/serverapi/event/total/alarmhis?epcode=${this.epcode}&alarmtype=1,4,11,12&begintime=${this.starttime}&endtime=${this.endtime}`,
    ...
    getAnamalyCurveTable = (selectedRows) => {
        ...
        propHisPromise.then((resArr) => {
            ...
            chart.xAxis[0].addPlotLine({
                // 在x轴上增加
                value: moment(selectedRows.starttime).valueOf(), // 在开始时间处
                width: 5, // 标示线的宽度为2px
     ...
     * @table表格部分
     */
    onTableRowclick = (e,record) => {
        console.log("onTableRowclick1019: ", e)
        console.log("record1019: ", record)
        let id = (Number(record.id) - 1).toString()
        let preId = this.state.rowkey
        if (id !== preId) {
            this.rowCheckSelect(id, [record])
        }
    ...
    componentDidUpdate =()=> {
        if (this.first === 0 && this.state.dataSource.length) {
            console.log('this.state.dataSource',this.state.dataSource)
            this.rowCheckSelect('0', this.state.dataSource)
            this.first = 1
        }
    ...
    componentDidMount = () => {
        ...
        if (oobj.starttime !== "") {
            ...
            // console.log(
            //     "alarmStartTime",
            //     moment(this.alarmStartTime).add(1, "hours").format("YYYY-MM-DD HH:mm:ss"),
            //     this.alarmSourceName
            // );
            this.endtime = moment(this.alarmStartTime).add(1, "hours").format("YYYY-MM-DD HH:mm:ss")
            ...
            this.setState({
                timeRangeValue: [
                    moment(
                        moment(this.alarmStartTime).subtract(12, "hours"),
                        "YYYY-MM-DD HH:mm:ss"
                    ),
                    moment(moment(this.alarmStartTime).add(1, "hours"), "YYYY-MM-DD HH:mm:ss")
                ]
            })
    ...
    render() {
        const { dataSource, dataParam, pagination, loading, paramLoading, timeRangeValue,rowkey } =
            this.state
        ...
        let rowSelection = {
            ...
            getCheckboxProps: (record) => {
                ...
                // console.log('选择了')
                /* return {
                    defaultChecked: record.id === "1" // 配置默认勾选的列 （Bealean）
                } */
                return {
                    defaultChecked: record.id === "00" // 配置默认勾选的列 （Bealean）
                }
                // return {}
            },
                    ...
                    <Table
                        ...
                        onRow={(record) => {
                            return {
                                // 点击行
                                onClick: (event) => {
                                    this.onTableRowclick(event,record)
                                }, // 双击行
                                // onDoubleClick: (event) => {
...
.ant-layout .ant-table-placeholder {
    display: block;
    padding: 0;
    line-height: 44px;
}

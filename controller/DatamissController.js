 ...
 * @LastEditTime: 2021-08-25 14:36:36
...
const getFactor = async function (router, params, factor) {
    ...
    var arr = [];
    if (jsonData.info.toLocaleLowerCase() !== 'success') {
        return arr;
    }
...
const getFactoryFactorsHis = async function (Epcode, EpcodeObjname, factoryName) {
    // TODO: 获取该企业所有关键参数
    let currentEpcode = Epcode;
    // TODO: ①查询企业所有关键参数("serverapi/data/rtd?epcode=xxx&mode=1")
    let params = { epcode: currentEpcode, mode: '1' };
    const factors = await getFactor('serverapi/data/rtd', JSON.stringify(params), 'name');
    const fatorsDesc = await getFactor('serverapi/data/rtd', JSON.stringify(params), 'description');
    if (factors.length === 0) {
    ...
    Object.keys(datahisResult).forEach((item, index) => {
        ...
        hisStatistic.push({
            ...
            serverTime: moment(datahisResult[item].tags.serverTime).format('HH:mm:ss'),
            originalTime: moment(datahisResult[item].tags.originalTime).format('HH:mm:ss')
        });
    });

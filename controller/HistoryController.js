 ...
 * @LastEditTime: 2021-08-23 20:46:37
...
const getTrend = async function (factory, factor, beginTime, endTime) {
    ...
    promiseall.forEach((item) => {
        ...
        let data =
            !item.error && item.info && item.info.toLowerCase() === 'success' ? item.data : {};
...
const getBatchQueryWhile = async function (objName, propName, beginTime, endTime) {
    let data = {
        list: [
            {
    ...
    if (factoriesRes.info.toLowerCase() === 'success') {
        ...
        datas.forEach((item) => {
            if (!epcodes.includes(item.epcode)) {
                let displayName = item.displayname.match(/.{2,}(处理厂|公司|南厂|水务)(\)|）)*/g);
...
const saveHisdata = function (hisdata) {
    ...
    XLSX_JSON.saveJsonToFile(
        hisdata,
        basicPath,
        `${moment(beginTime).format('YYYY-MM-DD_HH_mm_ss')}to${moment(endTime).format(
            'YYYY-MM-DD_HH_mm_ss'
        )}`
    );
...
historyController().then((result) => {
    console.log(result);
    console.log(
        `程序运行开始时间:${starttime}<==>结束时间:${moment().format('YYYY-MM-DD HH:mm:ss')}`
    );
    sendMessage(
        `程序运行开始时间:${starttime}<==>结束时间:${moment().format('YYYY-MM-DD HH:mm:ss')}`
    );
